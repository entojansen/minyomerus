...
%DIF DEL manuscript.tex           Fri May 18 14:53:44 2018
%DIF ADD revised_manuscript.tex   Fri Aug 10 01:55:21 2018
...
\section*{Introduction}\label{sec:intro}
	This phylogenetic \DIFdelbegin \DIFdel{revision }\DIFdelend \DIFaddbegin \DIFadd{study }\DIFaddend follows \citet{jansen2015} in the use of the taxonomic concept approach; see \citet{fp2009, fea2016a, fea2016b}. 
	Accordingly:
...
\section*{Materials and Methods}\label{sec:methods} 
	...
	\subsection*{Phylogenetic analysis}\label{ssec:phylo} 
		...
		\DIFdelbegin \DIFdel{The character sequence follows that of the taxonomic descriptions.
		}\DIFdelend \DIFaddbegin \DIFadd{Characters are numbered in accordance with descriptive sequence used in the species accounts.
		...
		Characters 9, 27, 39, 45 - 47, 49, and 51 were mapped onto the preferred phylogeny using ACCTRAN optimization (see \mbox{%DIFAUXCMD
\citealt{agnarsson2008}}\hspace{0pt}%DIFAUXCMD
), and the remaining characters had an unambiguous optimization.
		All multi-state characters but one were coded as additive, as explained beneath the description for each character (see }\textbf{\nameref{sec:results}}\DIFadd{), based on their alignment with the preferred phylogeny.
		...
		\DIFaddend The most parsimonious tree and character state optimizations were inferred under parsimony using NONA \citep{goloboff1999}.
	...
	\subsection*{Species distribution modeling}\label{ssec:dismo} 
		We used the modeling program Maxent, Version 3.4, to generate habitat models for the species of \DIFdelbegin \DIFdel{Minyomerus }\DIFdelend \DIFaddbegin \textit{\DIFadd{Minyomerus}} \DIFaddend [JF2018] (Figs.~\ref{fig:map_summary}-\ref{fig:map_sculptilis}) based on documented occurrence records \citep{phillips2004, phillips2006, elith2011}.
		...
		Documented occurrence records are laid over the modeled habitat ranges as colored circles on their respective maps (Figs.~\DIFdelbegin \DIFdel{\ref{fig:map_amptyl}}\DIFdelend \DIFaddbegin \DIFadd{\ref{fig:map_summary}}\DIFaddend -\ref{fig:map_sculptilis}), along with vector layers of country (white) and state (gray) borders \citep{hijmans2012}.
...
\section*{Descriptions of New Species}\label{sec:nsp} 
	\subsection*{\textit{Minyomerus ampullaceus} Jansen \& Franz sec. Jansen \& Franz, 2018; sp. n.}\label{ssec:amp}
		...
		\subsubsection*{Natural history}
			...
			The label indicates ``Geococcyx calif''; this is presumably a reference to \textit{Geococcyx californianus} (Lesson, 1829) [non-focal] (Cuculidae [non-focal]), the Greater Roadrunner\DIFdelbegin \DIFdel{, although it is unclear if the specimen was found on or near one of these birds (either living, dead, or in a nest).
			Species of }\textit{\DIFdel{Minyomerus}} %DIFAUXCMD
%DIFDELCMD < [%%%
\DIFdel{JF2018}%DIFDELCMD < ] %%%
\DIFdel{are only known to be phytophagous, not parasitic, phoretic, or necrophagous. Hence we believe that this }\DIFdelend \DIFaddbegin \DIFadd{. We had initially believed that this indicated a specimen found in a roadrunner nest; however, according to our reviewers, the USNM frequently assisted with the identification of insect specimens retrieved from the stomach contents of birds, and thus the }\DIFaddend specimen was most likely \DIFdelbegin \DIFdel{found in a nest, and was present there only incidentally because the nest was constructed in the host plant of this specimen\mbox{%DIFAUXCMD
\citep{jansen2015}}\hspace{0pt}%DIFAUXCMD
}\DIFdelend \DIFaddbegin \DIFadd{retrieved from the gut contents of a roadrunner. This seems quite likely given the poor external condition of the specimen}\DIFaddend .
...
region, setae covering 2/3 of dorsal surface area; dorsally with 7 apicomesal lacinial teeth; ventrally with 4 reduced lacinial teeth. 
		...
		\subsubsection*{Material examined}
			...
			\paragraph{Paratypes}
				...
				``MEXICO: San Luis Potosi; Entronque el Hulzache; 2 June 1987; R. Turnbow'' (\texttt{\DIFdelbegin \textbf{\DIFdel{CMNC}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \textbf{\DIFadd{USNM}}\DIFaddend }: 1 \female\DIFdelbegin \DIFdel{, }\DIFdelend \DIFaddbegin \DIFadd{; }\texttt{\textbf{\DIFadd{CMNC}}}\DIFadd{: }\DIFaddend 1 \male);
		...
		\subsubsection*{Natural history}
			...
			\DIFaddbegin \DIFadd{The indication of ``Sphaeralcea hastula A. Gray'' is not a valid name and appears to be a misspelling of }\textit{\DIFadd{Sphaeralcea hastulata}} [\DIFadd{non-focal}]\DIFadd{.
}\DIFaddend 
	...
	\subsection*{\textit{Minyomerus sculptilis} Jansen \& Franz sec. Jansen \& Franz, 2018; sp. n.}\label{ssec:sculpt}
		...
		\subsubsection*{Description of female}
			...
			\paragraph{Antennae}
				...
				Scape extending \DIFdelbegin \DIFdel{Brassicaceae  }\DIFdelend to posterior 1/4 of eye.
				Club nearly 3\x as long as wide.
			\paragraph{Head}
		...
		\subsubsection*{Material examined}
			...
			\paragraph{Paratypes}
				``Milner, Idaho; \#5a, 7-9-31; \textit{S.}[\textit{alsola}] \textit{pestifer}; David E. Fox'' (\texttt{\DIFdelbegin \textbf{\DIFdel{USNM}}%DIFAUXCMD
\DIFdelend \DIFaddbegin \textbf{\DIFadd{CMNC}}\DIFaddend }: 1 \female);
...
\DIFaddend \section*{Checklist of Species}\label{sec:checklist}
...
\DIFaddbegin \newpage
\DIFaddend \section*{Phylogenetic Results}\label{sec:results}
	...
	\DIFdelbegin \DIFdel{Characters are numbered in accordance with descriptive sequence used in the species accounts.
	A ``–'' symbol indicates inapplicable (character, state), whereas a ``?'' symbol indicates missing information, e.g., due to the unavailability of male specimens or insufficient specimens on hand to permit full dissections.
	Characters 9, 27, 39, 45 - 47, 49, and 51 were mapped onto the preferred phylogeny using ACCTRAN optimization (see \mbox{%DIFAUXCMD
\citealt{agnarsson2008}}\hspace{0pt}%DIFAUXCMD
), and the remaining characters had an unambiguous optimization.
	...
	\begin{enumerate}
		...
		\item Habitus, lateral elytral setae and ventral setae differentiated from setae of elytral disc: (0) absent; (1) present. Homoplasy for \textit{Minyomerus} [JF2018], with a reversal (state 0) in the \textit{M. aeriballux}--\textit{M. languidus} clade [JF2015], subsequent convergent gain (state 1) in the \textit{M. bulbifrons}--\textit{M. caseyi} clade [JF2018], and convergent reversal (state 0) in the \textit{M. bulbifrons}--\textit{M. puticalutus} clade [JF2015] (ci = 25; ri = 70).
	 	...
	 	\item Prementum, anterior margin \DIFdelbegin \DIFdel{forming a distinct face }\DIFdelend \DIFaddbegin \DIFadd{medially with a distinct facet, rather than a single edge edge, }\DIFaddend that continues to lateral margins: (0) absent; (1) present. Synapomorphy for the \textit{M. aeriballux}--\textit{M. languidus} clade [JF2015], with a single reversal in the  \textit{M. caseyi}--\textit{M. trisetosus} clade [JF2015] (ci = 50; ri = 75).
		...
		\item Legs, relative length of mesotarsi to mesotibiae: (0) tarsi \DIFdelbegin \DIFdel{much shorter than }\DIFdelend \DIFaddbegin \DIFadd{less than 3/4\x length of }\DIFaddend tibiae; (1) tarsi at least equal in length to tibiae; (2) tarsi \DIFdelbegin \DIFdel{slightly }\DIFdelend shorter than tibiae\DIFaddbegin \DIFadd{, but longer than 3/4\x length of tibiae}\DIFaddend . Coded as additive due to alignment of character states with preferred phylogeny. Coding as non-additive in isolation or in unison with other additive multi-state characters does not affect polarization of the character/states or alter the phylogeny. State 1 is a synapomorphy for the tanymecine clade [non-focal], whereas state 2 is a synapomorphy for \textit{Minyomerus} [JF2018].
...
\section*{RCC--5 Alignments}\label{sec:alignments}
	...
	We obtain high-level congruence among the concepts \textit{Minyomerus} [JF2018] and \textit{Minyomerus} [JF2015], where 17 species-level concepts are retained from \citet{jansen2015} and four species-level concepts are added in the current \DIFdelbegin \DIFdel{revision}\DIFdelend \DIFaddbegin \DIFadd{review}\DIFaddend .
...
\section*{Acknowledgments}
	The authors are grateful to Robert Anderson (CMNC), Ed Riley and John Oswald (TAMU), and Lourdes Chamorro (USNM) for their assistance and provision of specimens used in this \DIFdelbegin \DIFdel{revision}\DIFdelend \DIFaddbegin \DIFadd{study}\DIFaddend .
...
\DIFdelbegin \DIFdel{This research was funded in part by the National Science Foundation (DEB-1155984) and the United States Department of Agriculture -- Agricultural Research Service (Agreement 58-1275-1-335).
}\DIFdelend 
...
\newpage
%DIF > RCC-5 alignment visualizations
\DIFaddbegin \begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth]{figure1.png}
	\caption{\textbf{\DIFaddFL{Intensional RCC--5 alignment of the rank-only classifications of }\textit{\DIFaddFL{Minyomerus}} [\DIFaddFL{JF2018}]\DIFaddFL{/}[\DIFaddFL{JF2015}]\DIFaddFL{.}} \DIFaddFL{See also \mbox{%DIFAUXCMD
\citet{jansen2015} }\hspace{0pt}%DIFAUXCMD
and }\textbf{\DIFaddFL{Supplemental Information SI2}}\DIFaddFL{. Taxonomic concept labels such as }\textit{\DIFaddFL{Minymerus microps}} [\DIFaddFL{JF2015}] \DIFaddFL{are abbreviated as "2015.Minyomerus\_microps". Relaxation of the coverage constraint is indicated with the prefix "nc\_" (no coverage). Congruent concept regions (T\textsubscript{2} and T\textsubscript{1}) are shown as grey rectangles, concepts regions unique to the later taxonomy (T\textsubscript{2}) are shown as green rectangles, and concept regions unique to the earlier taxonomy (T\textsubscript{1}) are shown as yellow octagons. Articulations of inverse proper inclusion $(<)$ and overlap $(><)$, where present, are also shown.}}
	\label{fig:align1}
\end{figure}
\DIFaddend 
...
\DIFaddbegin \begin{figure}[h]
	...
	\caption{\textbf{\DIFaddFL{Intensional RCC--5 alignment of the phylogenies of }\textit{\DIFaddFL{Minyomerus}} [\DIFaddFL{JF2018}]\DIFaddFL{/}[\DIFaddFL{JF2015}] \DIFaddFL{-- whole-concept resolution with overlap.}} \DIFaddFL{See also }\textbf{\DIFaddFL{Supplemental Information SI3}}\DIFaddFL{. Seven overlapping articulations are inferred. For further discussion, see the }\textbf{\nameref{sec:alignments}} \DIFaddFL{section.}}
	\label{fig:align2}
\end{figure}
...
\begin{figure}[h]
	...
	\caption{\textbf{\DIFaddFL{Intensional RCC--5 alignment of the phylogenies of }\textit{\DIFaddFL{Minyomerus}} [\DIFaddFL{JF2018}]\DIFaddFL{/}[\DIFaddFL{JF2015}] \DIFaddFL{-- split-concept resolution.}} \DIFaddFL{See also }\textbf{\DIFaddFL{Supplemental Information SI4}}\DIFaddFL{. The seven overlapping articulations of the alignment displayed Fig.~\ref{fig:align2} are resolved into their constituent split regions. That is, if regions A and B overlap, the three resulting split regions are labeled A\textbackslash b ("A, }\emph{\DIFaddFL{not}} \DIFaddFL{b"), A*B ("A }\emph{\DIFaddFL{and}} \DIFaddFL{B"), and B\textbackslash a ("B, }\emph{\DIFaddFL{not}} \DIFaddFL{a"). Five split-concept regions can }\emph{\DIFaddFL{only}} \DIFaddFL{be named using this convention, and are salmon-colored in the alignment visualization.}}
	\label{fig:align3}
\end{figure}

%DIF > distribution maps
\begin{figure}[h]
	\centering
	\includegraphics[width=\textwidth]{figure4.png}
	\caption{\textbf{\DIFaddFL{Summary map of distributions of new species of }\textit{\DIFaddFL{Minyomerus}} [\DIFaddFL{JF2018}]\DIFaddFL{.}} \DIFaddFL{Combined occurrence record and Maxent habitat modeling map for four newly-described species of }\textit{\DIFaddFL{Minyomerus}} [\DIFaddFL{JF2018}]\DIFaddFL{, as indicated in the legend.}}
...
\begin{figure}[h]
	...
	\caption{\textbf{\DIFaddFL{Distributions of }\textit{\DIFaddFL{M. ampullaceus}} [\DIFaddFL{JF2018}] \DIFaddFL{and }\textit{\DIFaddFL{M. tylotos}} [\DIFaddFL{JF2018}]\DIFaddFL{.}} \DIFaddFL{Combined occurrence record and Maxent habitat modeling map for }\textit{\DIFaddFL{M. ampullaceus}} [\DIFaddFL{JF2018}] \DIFaddFL{and }\textit{\DIFaddFL{M. tylotos}} [\DIFaddFL{JF2018}]\DIFaddFL{, as indicated in the legend.}}
...
\begin{figure}[h]
	...
	\caption{\textbf{\DIFaddFL{Distributions of }\textit{\DIFaddFL{M. franko}} [\DIFaddFL{JF2018}]\DIFaddFL{.}} \DIFaddFL{Combined occurrence record and Maxent habitat modeling map for }\textit{\DIFaddFL{M. franko}} [\DIFaddFL{JF2018}]\DIFaddFL{, as indicated in the legend.}}
...
\begin{figure}[h]
	...
	\begin{sideways}
		...
		\end{sideways}
	\caption{\textbf{\DIFaddFL{Distributions of }\textit{\DIFaddFL{M. sculptilis}} [\DIFaddFL{JF2018}]\DIFaddFL{.}} \DIFaddFL{Combined occurrence record and Maxent habitat modeling map for }\textit{\DIFaddFL{M. sculptilis}} [\DIFaddFL{JF2018}]\DIFaddFL{, as indicated in the legend.}}
	\label{fig:map_sculptilis}
\end{figure}

\DIFaddend %M. ampullaceus
\begin{figure}[h]
	\begin{sideways}
		\centering
		\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=\textwidth]{figure1.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=\textwidth]{figure8.jpg}
	\DIFaddendFL \end{sideways}
	\caption{\textbf{Dorsal habitus of \textit{M. ampullaceus} [JF2018].} Image of female (\female) holotype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
...
\begin{figure}[h]
	\begin{sideways}
		\centering
		\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=\textwidth]{figure2.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=\textwidth]{figure9.jpg}
	\DIFaddendFL \end{sideways}
	\caption{\textbf{Lateral habitus of \textit{M. ampullaceus} [JF2018].} Image of female (\female) holotype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
...
\begin{figure}[h]
	\begin{sideways}
		\centering
		\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=\textwidth]{figure3.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=\textwidth]{figure10.jpg}
	\DIFaddendFL \end{sideways}
	\caption{\textbf{Ventral habitus of \textit{M. ampullaceus} [JF2018].} Image of female (\female) holotype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
	\label{fig:ampullaceus_F_ventral}
\end{figure}

\begin{figure}[h]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figure4.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figure11.jpg}
	\DIFaddendFL \caption{\textbf{Head and rostrum of \textit{M. ampullaceus} [JF2018].} Frontal view of female (\female) holotype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
	\label{fig:ampullaceus_F_frontal}
\end{figure}

\begin{figure}[h]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figure5.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figure12.jpg}
	\DIFaddendFL \caption{\textbf{Spermatheca of \textit{M. ampullaceus} [JF2018].} Genitalia of female (\female) holotype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
	\label{fig:ampullaceus_spermatheca}
\end{figure}

\begin{figure}[h]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figure6.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figure13.jpg}
	\DIFaddendFL \caption{\textbf{Lamina of spiculum ventrale of \textit{M. ampullaceus} [JF2018].} Sternum VIII of female (\female) holotype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
...
\begin{figure}[h]
	\begin{sideways}
		\centering
		\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=\textwidth]{figure7.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=\textwidth]{figure14.jpg}
	\DIFaddendFL \end{sideways}
	\caption{\textbf{Dorsal habitus of \textit{M. franko} [JF2018].} Image of female (\female) holotype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
...
\begin{figure}[h]
	\begin{sideways}
		\centering
		\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=\textwidth]{figure8.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=\textwidth]{figure15.jpg}
	\DIFaddendFL \end{sideways}
	\caption{\textbf{Lateral habitus of \textit{M. franko} [JF2018].} Image of female (\female) holotype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
...
\begin{figure}[h]
	\begin{sideways}
		\centering
		\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=\textwidth]{figure9.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=\textwidth]{figure16.jpg}
	\DIFaddendFL \end{sideways}
	\caption{\textbf{Ventral habitus of \textit{M. franko} [JF2018].} Image of female (\female) holotype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
	\label{fig:franko_F_ventral}
\end{figure}

\begin{figure}[h]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figure10.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figure17.jpg}
	\DIFaddendFL \caption{\textbf{Head and rostrum of \textit{M. franko} [JF2018].} Frontal view of female (\female) holotype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
	\label{fig:franko_F_frontal}
\end{figure}

\begin{figure}[h]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figure11.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figure18.jpg}
	\DIFaddendFL \caption{\textbf{Maxilla of \textit{M. franko} [JF2018].} Dextral maxilla of female (\female) paratype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
	\label{fig:franko_maxilla}
\end{figure}

\begin{figure}[h]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figure12.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figure19.jpg}
	\DIFaddendFL \caption{\textbf{Prementum of \textit{M. franko} [JF2018].} Labium of female (\female) paratype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
	\label{fig:franko_prementum}
\end{figure}

\begin{figure}[h]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figure13.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figure20.jpg}
	\DIFaddendFL \caption{\textbf{Spermatheca of \textit{M. franko} [JF2018].} Genitalia of female (\female) paratype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
	\label{fig:franko_spermatheca}
\end{figure}

\begin{figure}[h]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figure14.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figure21.jpg}
	\DIFaddendFL \caption{\textbf{Lamina of spiculum ventrale of \textit{M. franko} [JF2018].} Sternum VIII of female (\female) paratype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
...
\begin{figure}[h]
	\centering
	\begin{sideways}
		\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\textheight]{figure15.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\textheight]{figure22.jpg}
	\DIFaddendFL \end{sideways}
	\caption{\textbf{Aedeagus of \textit{M. franko} [JF2018].} Genitalia of male (\male) paratype in (A) dorsal view and (B) lateral view. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
...
\begin{figure}[h]
	\begin{sideways}
		\centering
		\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=\textwidth]{figure16.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=\textwidth]{figure23.jpg}
	\DIFaddendFL \end{sideways}
	\caption{\textbf{Dorsal habitus of \textit{M. sculptilis} [JF2018].} Image of female (\female) holotype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
...
\begin{figure}[h]
	\begin{sideways}
		\centering
		\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=\textwidth]{figure17.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=\textwidth]{figure24.jpg}
	\DIFaddendFL \end{sideways}
	\caption{\textbf{Lateral habitus of \textit{M. sculptilis} [JF2018].} Image of female (\female) holotype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
...
\begin{figure}[h]
	\begin{sideways}
		\centering
		\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=\textwidth]{figure18.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=\textwidth]{figure25.jpg}
	\DIFaddendFL \end{sideways}
	\caption{\textbf{Ventral habitus of \textit{M. sculptilis} [JF2018].} Image of female (\female) holotype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
	\label{fig:sculptilis_F_ventral}
\end{figure}

\begin{figure}[h]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figure19.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figure26.jpg}
	\DIFaddendFL \caption{\textbf{Head and rostrum of \textit{M. sculptilis} [JF2018].} Frontal view of female (\female) holotype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
	\label{fig:sculptilis_F_frontal}
\end{figure}

\begin{figure}[h]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figure20.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figure27.jpg}
	\DIFaddendFL \caption{\textbf{Spermatheca of \textit{M. sculptilis} [JF2018].} Genitalia of female (\female) paratype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
	\label{fig:sculptilis_spermatheca}
\end{figure}

\begin{figure}[h]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figure21.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figure28.jpg}
	\DIFaddendFL \caption{\textbf{Lamina of spiculum ventrale of \textit{M. sculptilis} [JF2018].} Sternum VIII of female (\female) paratype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
...
\begin{figure}[h]
	\centering
	\begin{sideways}
		\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=0.95\textheight]{figure22.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=0.95\textheight]{figure29.jpg}
	\DIFaddendFL \end{sideways}
	\caption{\textbf{Aedeagus of \textit{M. sculptilis} [JF2018].} Genitalia of male (\male) paratype in (A) dorsal view and (B) lateral view. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
...
\begin{figure}[h]
	\begin{sideways}
		\centering
		\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=\textwidth]{figure23.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=\textwidth]{figure30.jpg}
	\DIFaddendFL \end{sideways}
	\caption{\textbf{Dorsal habitus of \textit{M. tylotos} [JF2018].} Image of female (\female) holotype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
...
\begin{figure}[h]
	\begin{sideways}
		\centering
		\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=\textwidth]{figure24.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=\textwidth]{figure31.jpg}
	\DIFaddendFL \end{sideways}
	\caption{\textbf{Lateral habitus of \textit{M. tylotos} [JF2018].} Image of female (\female) holotype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
...
\begin{figure}[h]
	\begin{sideways}
		\centering
		\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=\textwidth]{figure25.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=\textwidth]{figure32.jpg}
	\DIFaddendFL \end{sideways}
	\caption{\textbf{Ventral habitus of \textit{M. tylotos} [JF2018].} Image of female (\female) holotype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
	\label{fig:tylotos_F_ventral}
\end{figure}

\begin{figure}[h]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figure26.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figure33.jpg}
	\DIFaddendFL \caption{\textbf{Head and rostrum of \textit{M. tylotos} [JF2018].} Frontal view of female (\female) holotype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
	\label{fig:tylotos_F_frontal}
\end{figure}

\begin{figure}[h]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figure27.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figure34.jpg}
	\DIFaddendFL \caption{\textbf{Prementum of \textit{M. tylotos} [JF2018].} Labium of female (\female) paratype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
	\label{fig:tylotos_prementum}
\end{figure}

\begin{figure}[h]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figure28.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figure35.jpg}
	\DIFaddendFL \caption{\textbf{Spermatheca of \textit{M. tylotos} [JF2018].} Genitalia of female (\female) paratype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
	\label{fig:tylotos_spermatheca}
\end{figure}

\begin{figure}[h]
	\centering
	\DIFdelbeginFL %DIFDELCMD < \includegraphics[width=\textwidth]{figure29.jpg}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[width=\textwidth]{figure36.jpg}
	\DIFaddendFL \caption{\textbf{Lamina of spiculum ventrale of \textit{M. tylotos} [JF2018].} Sternum VIII of female (\female) paratype. \DIFaddbeginFL \DIFaddFL{Photo credit: Andrew Jansen.}\DIFaddendFL }
...
\begin{figure}[h]
	\centering
	\begin{sideways}
		\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=0.85\textwidth]{figure30.png}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=0.85\textwidth]{figure37.png}
	\DIFaddendFL \end{sideways}
...
\begin{figure}[h]
	\centering
	\begin{sideways}
		\DIFdelbeginFL %DIFDELCMD < \includegraphics[height=0.85\textwidth]{figure31.png}
%DIFDELCMD < 	%%%
\DIFdelendFL \DIFaddbeginFL \includegraphics[height=0.85\textwidth]{figure38.png}
	\DIFaddendFL \end{sideways}
...
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\includegraphics[width=\textwidth]{figure32.png}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\textbf{\DIFdelFL{Intensional RCC--5 alignment of the rank-only classifications of }\textit{\DIFdelFL{Minyomerus}} %DIFAUXCMD
%DIFDELCMD < [%%%
\DIFdelFL{JF2018}%DIFDELCMD < ]%%%
\DIFdelFL{/}%DIFDELCMD < [%%%
\DIFdelFL{JF2015}%DIFDELCMD < ]%%%
\DIFdelFL{.}} %DIFAUXCMD
\DIFdelFL{See also \mbox{%DIFAUXCMD
\citet{jansen2015} }\hspace{0pt}%DIFAUXCMD
and }\textbf{\DIFdelFL{Supplemental Information SI2}}%DIFAUXCMD
\DIFdelFL{. Taxonomic concept labels such as }\textit{\DIFdelFL{Minymerus microps}} %DIFAUXCMD
%DIFDELCMD < [%%%
\DIFdelFL{JF2015}%DIFDELCMD < ] %%%
\DIFdelFL{are abbreviated as "2015.Minyomerus\_microps". Relaxation of the coverage constraint is indicated with the prefix "nc\_" (no coverage). Congruent concept regions (T\textsubscript{2} and T\textsubscript{1}) are shown as grey rectangles, concepts regions unique to the later taxonomy (T\textsubscript{2}) are shown as green rectangles, and concept regions unique to the earlier taxonomy (T\textsubscript{1}) are shown as yellow octagons. Articulations of inverse proper inclusion $(<)$ and overlap $(><)$, where present, are also shown.}}
	%DIFAUXCMD
%DIFDELCMD < \label{fig:align1}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[h]
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\includegraphics[width=\textwidth]{figure33.png}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\textbf{\DIFdelFL{Intensional RCC--5 alignment of the phylogenies of }\textit{\DIFdelFL{Minyomerus}} %DIFAUXCMD
%DIFDELCMD < [%%%
\DIFdelFL{JF2018}%DIFDELCMD < ]%%%
\DIFdelFL{/}%DIFDELCMD < [%%%
\DIFdelFL{JF2015}%DIFDELCMD < ] %%%
\DIFdelFL{-- whole-concept resolution with overlap.}} %DIFAUXCMD
\DIFdelFL{See also }\textbf{\DIFdelFL{Supplemental Information SI3}}%DIFAUXCMD
\DIFdelFL{. Seven overlapping articulations are inferred. For further discussion, see the }\textbf{%DIFDELCMD < \nameref{sec:alignments}%%%
} %DIFAUXCMD
\DIFdelFL{section.}}
	%DIFAUXCMD
%DIFDELCMD < \label{fig:align2}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[h]
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\includegraphics[width=\textwidth]{figure34.png}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\textbf{\DIFdelFL{Intensional RCC--5 alignment of the phylogenies of }\textit{\DIFdelFL{Minyomerus}} %DIFAUXCMD
...
\DIFdelFL{/}%DIFDELCMD < [%%%
\DIFdelFL{JF2015}%DIFDELCMD < ] %%%
\DIFdelFL{-- split-concept resolution.}} %DIFAUXCMD
\DIFdelFL{See also }\textbf{\DIFdelFL{Supplemental Information SI4}}%DIFAUXCMD
\DIFdelFL{. The seven overlapping articulations of the alignment displayed Fig.~\ref{fig:align2} are resolved into their constituent split regions. That is, if regions A and B overlap, the three resulting split regions are labeled A\textbackslash b ("A, }\emph{\DIFdelFL{not}} %DIFAUXCMD
\DIFdelFL{b"), A*B ("A }\emph{\DIFdelFL{and}} %DIFAUXCMD
\DIFdelFL{B"), and B\textbackslash a ("B, }\emph{\DIFdelFL{not}} %DIFAUXCMD
\DIFdelFL{a"). Five split-concept regions can }\emph{\DIFdelFL{only}} %DIFAUXCMD
\DIFdelFL{be named using this convention, and are salmon-colored in the alignment visualization.}}
	%DIFAUXCMD
%DIFDELCMD < \label{fig:align3}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 
...
%DIFDELCMD < \begin{figure}[h]
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\includegraphics[width=\textwidth]{figure35.png}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\textbf{\DIFdelFL{Summary map of distributions of new species of }\textit{\DIFdelFL{Minyomerus}} %DIFAUXCMD
%DIFDELCMD < [%%%
\DIFdelFL{JF2018}%DIFDELCMD < ]%%%
\DIFdelFL{.}} %DIFAUXCMD
\DIFdelFL{Combined occurrence record and Maxent habitat modeling map for four newly-described species of }\textit{\DIFdelFL{Minyomerus}} %DIFAUXCMD
%DIFDELCMD < [%%%
\DIFdelFL{JF2018}%DIFDELCMD < ]%%%
\DIFdelFL{, as indicated in the legend.}}
	%DIFAUXCMD
%DIFDELCMD < \label{fig:map_summary}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[h]
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\includegraphics[width=\textwidth]{figure36.png}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\textbf{\DIFdelFL{Distributions of }\textit{\DIFdelFL{M. ampullaceus}} %DIFAUXCMD
%DIFDELCMD < [%%%
\DIFdelFL{JF2018}%DIFDELCMD < ] %%%
\DIFdelFL{and }\textit{\DIFdelFL{M. tylotos}} %DIFAUXCMD
%DIFDELCMD < [%%%
\DIFdelFL{JF2018}%DIFDELCMD < ]%%%
\DIFdelFL{.}} %DIFAUXCMD
\DIFdelFL{Combined occurrence record and Maxent habitat modeling map for }\textit{\DIFdelFL{M. ampullaceus}} %DIFAUXCMD
%DIFDELCMD < [%%%
\DIFdelFL{JF2018}%DIFDELCMD < ] %%%
\DIFdelFL{and }\textit{\DIFdelFL{M. tylotos}} %DIFAUXCMD
%DIFDELCMD < [%%%
...
\DIFdelFL{, as indicated in the legend.}}
	%DIFAUXCMD
%DIFDELCMD < \label{fig:map_amptyl}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[h]
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\includegraphics[width=\textwidth]{figure37.png}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\textbf{\DIFdelFL{Distributions of }\textit{\DIFdelFL{M. franko}} %DIFAUXCMD
%DIFDELCMD < [%%%
\DIFdelFL{JF2018}%DIFDELCMD < ]%%%
\DIFdelFL{.}} %DIFAUXCMD
\DIFdelFL{Combined occurrence record and Maxent habitat modeling map for }\textit{\DIFdelFL{M. franko}} %DIFAUXCMD
%DIFDELCMD < [%%%
\DIFdelFL{JF2018}%DIFDELCMD < ]%%%
\DIFdelFL{, as indicated in the legend.}}
	%DIFAUXCMD
%DIFDELCMD < \label{fig:map_franko}
%DIFDELCMD < \end{figure}
%DIFDELCMD < 

%DIFDELCMD < \begin{figure}[h]
%DIFDELCMD < 	\centering
%DIFDELCMD < 	\begin{sideways}
%DIFDELCMD < 		\includegraphics[height=0.85\textwidth]{figure38.png}
%DIFDELCMD < 		\end{sideways}
%DIFDELCMD < 	%%%
%DIFDELCMD < \caption{%
{%DIFAUXCMD
\textbf{\DIFdelFL{Distributions of }\textit{\DIFdelFL{M. sculptilis}} %DIFAUXCMD
%DIFDELCMD < [%%%
\DIFdelFL{JF2018}%DIFDELCMD < ]%%%
\DIFdelFL{.}} %DIFAUXCMD
\DIFdelFL{Combined occurrence record and Maxent habitat modeling map for }\textit{\DIFdelFL{M. sculptilis}} %DIFAUXCMD
%DIFDELCMD < [%%%
\DIFdelFL{JF2018}%DIFDELCMD < ]%%%
\DIFdelFL{, as indicated in the legend.}}
	%DIFAUXCMD
%DIFDELCMD < \label{fig:map_sculptilis}
%DIFDELCMD < %%%
